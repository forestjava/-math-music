...
export function buildExactRetryPrompt(bannedExactFragments: string[]): string {
  ...
  return (
    `speech содержит запрещённые точные фрагменты из прошлых ответов: ${list}. ` +
    `Полная перезапись JSON: новый speech с нуля без этих фрагментов; ` +
    `StoryState перезапиши целиком под новую сцену (не дополняй прошлые списки); ` +
    `whatChanged = факт, буквально слышный в новом speech; ` +
    `сдвинь первый openThreads новым событием.`
  );
}
