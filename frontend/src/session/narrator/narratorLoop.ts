import { decodeNarratorBuffer, fetchNarratorAudio } from "../../audio/narrator/narratorClient";
...
export class NarratorLoop {
  ...
  /** Старт из stopped: вступление (обязательно) → эффекты → основной цикл. */
  async start(): Promise<void> {
    ...
    void this.runMainLoop(generation);
  }
  ...
  /** Завершение сессии (!loop): заключительное слово (обязательно) → финиш. */
  ...
  private async speakSafely(prompt: string, speed: number): Promise<boolean> {
    try {
      const bytes = await fetchNarratorAudio(prompt, speed);
